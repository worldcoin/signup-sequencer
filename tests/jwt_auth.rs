...
fn generate_es256_keypair() -> (String, String) {
    ...
    let private_key_pem =
        String::from_utf8(private_key_output.stdout).expect("Invalid UTF-8 in private key");
    ...
    let output = public_key_process
        .wait_with_output()
        .expect("Failed to extract public key");

    let public_key_pem = String::from_utf8(output.stdout).expect("Invalid UTF-8 in public key");
